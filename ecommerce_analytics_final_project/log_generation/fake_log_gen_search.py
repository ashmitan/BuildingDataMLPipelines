...
outFileName = 'search_log_' + timestr + '.log' if not file_prefix else file_prefix + '_access_log_' + timestr + '.log'
...
resources = ["/explore", "/search/tag/list"]
...
while flag:
    ...
    if log_format == "CLF":
        ...
        f.write(
            '%s - - [%s %s] "%s %s HTTP/1.0" %s %s "%s" "%s" "%s" "%s"  \n' % (
             ip, dt, tz, vrb, uri, resp, byt,
            referer, useragent, prod_name, prod_category))
    f.flush()
