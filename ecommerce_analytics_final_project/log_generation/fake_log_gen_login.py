...
# timestr = datetime.datetime.strftime(datetime.datetime.now() - datetime.timedelta(), "%Y%m%d-%H%M%S")

timestr = time.strftime("%Y%m%d-%H%M%S")
otime = datetime.datetime.now()

outFileName = 'member_access_log_' + timestr + '.log' if not file_prefix else file_prefix + '_access_log_' + timestr + '.log'
...
resources = ["/user/register", "/user/login"]
...
while flag:
    ...
    cust_username = cust_details[6]
    cust_email = cust_details[3]
    gen = cust_details[5]
    ...
    if log_format == "CLF":
        ...
        f.write(
            '%s - - [%s %s] "%s %s HTTP/1.0" %s %s "%s" "%s" "%s" "%s" %s %s \n' % (
             ip, dt, tz, vrb, uri, resp, byt,
            referer, useragent,cust_nm, cust_username, cust_email, gen))
    f.flush()
